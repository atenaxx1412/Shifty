...
export default function StaffContactForm({ onSubmitSuccess }: { onSubmitSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // お問い合わせ送信（スタッフ用にfromRoleを'staff'に変更）
      const timestamp = new Date();
      await addDoc(collection(db, 'inquiries'), {
        timestamp,
        fromRole: 'staff', // ここを'manager'から'staff'に変更
      ...
      console.log('✅ スタッフお問い合わせが送信され、履歴に保存されました');
    } catch (error) {
      console.error('❌ Error submitting staff inquiry:', error);
