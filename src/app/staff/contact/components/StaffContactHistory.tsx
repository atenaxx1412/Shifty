...
export default function StaffContactHistory({ refreshTrigger }: ContactHistoryProps) {
  ...
  useEffect(() => {
    const loadInquiries = () => {
      if (!currentUser) {
        console.log('📋 No current user, skipping staff inquiry load');
        setLoading(false);
        return;
      }

      console.log('📋 Loading staff inquiries from localStorage for user:', currentUser.uid);

      try {
        const inquiriesData = ContactHistoryStorage.getHistory(currentUser.uid);
        setInquiries(inquiriesData);
        console.log(`📋 Loaded ${inquiriesData.length} staff inquiries from localStorage`);
      } catch (error) {
        console.error('❌ Error loading staff inquiries from localStorage:', error);
      } finally {
        setLoading(false);
      }
    };
