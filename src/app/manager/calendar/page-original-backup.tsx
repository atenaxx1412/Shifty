...
import {
  ...
  addJapaneseFonts,
  formatJapaneseDate,
  formatJapaneseMonthYear,
} from "@/lib/japanesePdfFonts";
...
export default function ManagerCalendarPage() {
  ...
  const [selectedStaffForCalendar, setSelectedStaffForCalendar] = useState<
    string[]
  >([]);
  ...
  const [selectedShift, setSelectedShift] = useState<ShiftExtended | null>(
    null
  );
  const [selectedSlot, setSelectedSlot] = useState<ShiftSlot | null>(null);
  const [selectedShiftStaff, setSelectedShiftStaff] = useState<User | null>(
    null
  );

  // 人員テンプレート関連の状態
  const [staffingTemplate, setStaffingTemplate] =
    useState<StaffingTemplate | null>(null);
  const [templateLoading, setTemplateLoading] = useState(false);

  // 開発用：サンプルスタッフデータを作成
  const createSampleStaffData = async () => {
    if (
      !confirm(`サンプルスタッフデータを作成しますか？
（既存データがある場合は上書きされます）`)
    ) {
      return;
    }
...
${sampleStaff.length}名のスタッフが追加されました。`);
      ...
      alert(
        "サンプルデータの作成に失敗しました。コンソールを確認してください。"
      );
    }
  };
  ...
  useEffect(() => {
    setLoading(true);
    console.log(
      "📅 Setting up real-time shift subscription for manager:",
      managerUser.uid
    );
  ...
  useEffect(() => {
    console.log(
      "👥 Setting up staff data subscription for manager:",
      managerUser.uid
    );
    ...
    const unsubscribeStaff = userService.subscribeToStaffUpdates(
      ...
      (staffData) => {
        console.log(
          `📊 Received ${staffData.length} staff members from Firestore`
        );
        setStaff(staffData);
        ...
        if (staffData.length === 0) {
          console.log(
            "⚠️ No staff found for this manager. Please add staff members."
          );
        }
      }
    );
  ...
  // 人員テンプレートの読み込み（最適化版）
  useEffect(() => {
    const loadStaffingTemplate = async () => {
      if (!managerUser?.uid) return;

      setTemplateLoading(true);
      try {
        const month = format(selectedDate, "yyyy-MM");
        console.log("🔄 Loading optimized staffing template for month:", month);
        const template = await ManagerDataService.getOptimizedTemplate(
          managerUser.uid,
          month
        );
        setStaffingTemplate(template);
        console.log(
          "📋 Loaded staffing template for month:",
          month,
          template ? "✅" : "❌"
        );
      } catch (error) {
        console.error("❌ Error loading staffing template:", error);
        setStaffingTemplate(null);
      } finally {
        setTemplateLoading(false);
      }
    };

    loadStaffingTemplate();
  }, [managerUser.uid, selectedDate]); // managerUser.uidとselectedDateに依存

  // 選択されたスタッフの時間設定を同期
  useEffect(() => {
    // 現在選択されているスタッフと時間設定を同期
    const newStaffTimeSettings: {
      [staffId: string]: {
        startTime: string;
        endTime: string;
        positions: string;
    ...
    selectedStaffForCalendar.forEach((staffId) => {
      // 既存の設定があれば保持、なければデフォルト値
      if (!staffTimeSettings[staffId]) {
        newStaffTimeSettings[staffId] = {
          startTime: "09:00",
          endTime: "17:00",
          positions: "",
          notes: "",
        ...
        newStaffTimeSettings[staffId] = staffTimeSettings[staffId];
      }
    });

    // 変更があった場合のみ更新
    const hasChanges =
      Object.keys(newStaffTimeSettings).length !==
        Object.keys(staffTimeSettings).length ||
      selectedStaffForCalendar.some((staffId) => !staffTimeSettings[staffId]);

    if (hasChanges) {
      setStaffTimeSettings(newStaffTimeSettings);
    }
  }, [selectedStaffForCalendar, staffTimeSettings]); // 必要な依存のみ
  ...
  const getDayStats = (date: Date) => {
    ...
    // 人員テンプレートから推奨人数を取得
    let templateRequiredStaff = 0;
    let templateShortageByTimeSlot: { [key: string]: number } = {};
    let hasTemplateData = false;

    if (staffingTemplate) {
      try {
        const templateRequirement =
          StaffingTemplateService.getRequiredStaffForDate(
            staffingTemplate,
            date
          );
        templateRequiredStaff =
          templateRequirement.morning +
          templateRequirement.afternoon +
          templateRequirement.evening;
        hasTemplateData = true;

        // 時間帯別の不足分析
        const actualStaffByTimeSlot = {
          morning: 0,
          afternoon: 0,
          evening: 0,
        };
        ...
        dayShifts.forEach((shift) => {
          shift.slots.forEach((slot) => {
            ...
            actualStaffByTimeSlot[timeSlot] += slot.assignedStaff?.length || 0;
          });
        });
        ...
        templateShortageByTimeSlot = {
          morning: Math.max(
            0,
            templateRequirement.morning - actualStaffByTimeSlot.morning
          ),
          afternoon: Math.max(
            0,
            templateRequirement.afternoon - actualStaffByTimeSlot.afternoon
          ),
          evening: Math.max(
            0,
            templateRequirement.evening - actualStaffByTimeSlot.evening
          ),
        };
      } catch (error) {
        console.warn("テンプレート分析エラー:", error);
      }
    }

    const shortage = totalRequiredStaff - totalAssignedStaff;
    const templateShortage = Math.max(
      0,
      templateRequiredStaff - totalAssignedStaff
    );
    const fulfillmentRate =
      totalRequiredStaff > 0
        ? (totalAssignedStaff / totalRequiredStaff) * 100
        : 0;
    const templateFulfillmentRate =
      templateRequiredStaff > 0
        ? (totalAssignedStaff / templateRequiredStaff) * 100
        : 0;
    ...
    return {
      ...
      templateRequiredStaff,
      templateShortage,
      templateFulfillmentRate,
      templateShortageByTimeSlot,
      hasTemplateData,
      // 重要度判定
      isCriticalShortage:
        templateShortage >= 3 ||
        Object.values(templateShortageByTimeSlot).some((s) => s >= 2),
      isWarningShortage:
        templateShortage >= 1 ||
        Object.values(templateShortageByTimeSlot).some((s) => s >= 1),
    };
  };
  ...
  const getDateRangeLabel = () => {
    switch (calendarView) {
      ...
      case "month":
      default:
        return `${format(monthStart, "yyyy年 M月d日(E)", {
          locale: ja,
        })}〜${format(monthEnd, "M月d日(E)", { locale: ja })}`;
    }
  };
  ...
  const handleOpenStaffChat = (
    ...
    console.log(
      "💬 Opening chat with:",
      staffName,
      relatedShiftId ? `(Shift: ${relatedShiftId})` : ""
    );
  };
  ...
  const handleDeleteShiftSlot = async (slotId: string) => {
    ...
    try {
      ...
      const updatedSlots = selectedShift.slots.filter(
  ...
  const executeCreateShift = async () => {
    ...
    try {
      ...
      if (calendarView === "grid" && selectedStaffForCalendar.length > 0) {
        // カレンダー形式: 複数スタッフ選択
      ...
      // 複数スタッフが選択されており、個別時間設定がある場合
      if (
        calendarView === "grid" &&
        selectedStaffForCalendar.length > 0 &&
        Object.keys(staffTimeSettings).length > 0
      ) {
        // 各スタッフごとに個別のシフトを作成
        for (const staffId of selectedStaffForCalendar) {
          const staffSetting = staffTimeSettings[staffId];
          if (!staffSetting) continue;

          // 時間の妥当性チェック
          const start = new Date(`2000-01-01T${staffSetting.startTime}`);
          const end = new Date(`2000-01-01T${staffSetting.endTime}`);
          if (start >= end) {
            alert(
              `${
                staff.find((s) => s.uid === staffId)?.name || ""
              }さんの終了時間は開始時間より後に設定してください`
            );
            return;
          }

          // ポジションを配列に変換
          const positions = staffSetting.positions
            .split(",")
            .map((p) => p.trim())
            .filter((p) => p.length > 0);
          ...
          // 個別シフトデータを構築
          const individualShiftData = {
            managerId: managerUser.uid,
            date: createModalDate,
            slots: [
              {
                slotId: `slot_${staffId}_${Date.now()}`,
        ...
        // 作成完了メッセージ
        const staffNames = selectedStaffForCalendar
          .map((staffId) => {
        ...
        alert(`✅ ${staffNames}さんの${format(createModalDate, "M月d日", {
          locale: ja,
        })}シフトを個別時間で設定しました！
...
カレンダーに直ちに反映されています。`);
  ...
  // シフト出力の実行
  const executeExport = (
    exportFormat: "daily" | "weekly" | "monthly" | "pdf"
  ) => {
    ...
    if (exportFormat === "pdf") {
      // PDF出力の処理
      const fileName = `シフト表_${format(startDate, "yyyyMMdd")}-${format(
        endDate,
        "yyyyMMdd"
      )}_${staffName}.pdf`;
      ...
      // PDFサービスを呼び出す（jsPDFを使用）
      generatePDFExport(exportData, fileName);
    } else {
      // Excel出力の処理
      const fileName = `シフト表_${format(startDate, "yyyyMMdd")}-${format(
        endDate,
    ...
    closeExportModal();
    console.log(
      `Exported ${exportFormat} shift table:`,
      exportFormat === "pdf" ? "PDF" : "Excel"
    );
  };

  // PDF出力関数 - カレンダーグリッド形式（画面表示と同じ）
  const generatePDFExport = (exportData: any, fileName: string) => {
    const pdf = new jsPDF({
    ...
    // 日本語フォントを追加
    addJapaneseFonts(pdf);

    // タイトルを追加
    const title = `シフト表 - ${formatJapaneseMonthYear(
      exportData.selectedDate
    )}`;
    drawJapaneseText(pdf, title, 10, 15, { fontSize: 14, fontStyle: "bold" });

    // 期間を追加
    const periodText = `期間: ${formatJapaneseDate(
      exportData.startDate
    )} 〜 ${formatJapaneseDate(exportData.endDate)}`;
    drawJapaneseText(pdf, periodText, 10, 25, { fontSize: 10 });

    // 現在の画面表示と同じカレンダーグリッドを生成
    generateCalendarGridPDF(pdf, exportData);

    // 出力日時を追加
    const outputTime = `出力日時: ${format(new Date(), "yyyy年MM月dd日 HH:mm", {
      locale: ja,
    })}`;
    drawJapaneseText(pdf, outputTime, 10, pdf.internal.pageSize.height - 8, {
      fontSize: 8,
    });
  ...
  // カレンダーグリッド形式のPDF生成（画面表示と同じレイアウト）
  const generateCalendarGridPDF = (pdf: jsPDF, exportData: any) => {
    const { staff, shifts, startDate, endDate, calendarView } = exportData;

    // 表示する日付の配列を生成
    let displayDates: Date[] = [];
    if (calendarView === "day") {
      displayDates = [exportData.selectedDate];
    } else if (calendarView === "week") {
      const weekStart = startOfWeek(exportData.selectedDate, {
        weekStartsOn: 1,
      });
      displayDates = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i));
    } else {
      // 月表示の場合
      const monthStart = startOfMonth(exportData.selectedDate);
      const monthEnd = endOfMonth(exportData.selectedDate);
      const daysInMonth = getDaysInMonth(exportData.selectedDate);
      displayDates = Array.from({ length: daysInMonth }, (_, i) =>
        addDays(monthStart, i)
      );
    }
    ...
    const startY = 40;
    const cellWidth =
      calendarView === "day" ? 50 : calendarView === "week" ? 35 : 8;
    const cellHeight = 8;
    const staffNameWidth = 30;

    // 日付ヘッダーを描画
    drawJapaneseText(pdf, "スタッフ", startX, startY, {
      fontSize: 9,
      fontStyle: "bold",
    });

    displayDates.forEach((date, index) => {
      const xPos = startX + staffNameWidth + index * cellWidth;
      let dateText: string;

      if (calendarView === "day") {
        dateText = format(date, "M/d(E)", { locale: ja });
      } else if (calendarView === "week") {
        dateText = format(date, "M/d", { locale: ja });
      } else {
        dateText = date.getDate().toString();
      }

      drawJapaneseText(pdf, dateText, xPos, startY, {
        fontSize: 8,
        fontStyle: "bold",
      });
    });
    ...
    staff.forEach((staffMember: User, staffIndex: number) => {
      // スタッフ名を描画
      drawJapaneseText(pdf, staffMember.name, startX, currentY, {
        fontSize: 8,
        fontStyle: "bold",
      });
      ...
      displayDates.forEach((date, dateIndex) => {
        const xPos = startX + staffNameWidth + dateIndex * cellWidth;
        ...
        const dayShifts = shifts.filter(
          (shift: ShiftExtended) =>
            ...
            shift.slots.some((slot) =>
              slot.assignedStaff?.includes(staffMember.uid)
            )
        );
        ...
        if (dayShifts.length > 0) {
          ...
          const times = dayShifts
            .map((shift) =>
              shift.slots
                .filter((slot) => slot.assignedStaff?.includes(staffMember.uid))
                .map(
                  (slot) =>
        ...
        // セルに枠線を描画
        pdf.rect(xPos - 1, currentY - 3, cellWidth, cellHeight);

        // セル内にテキストを描画
        drawJapaneseText(pdf, cellText, xPos + 1, currentY, { fontSize: 6 });
      });
    ...
    // 表の外枠を描画
    const tableWidth = staffNameWidth + displayDates.length * cellWidth;
    const tableHeight = (staff.length + 1) * cellHeight;
    pdf.rect(startX - 1, startY - 3, tableWidth, tableHeight);
  };
  ...
  // 月間統計の計算（テンプレート統合版）
  const monthStats = monthDates.reduce(
    (acc, date) => {
                        ...
                        style={{
                          minWidth:
                            calendarView === "week"
                              ? "100%"
                              : `${
                                  monthDates.length * cellSizes.minCellWidth
                                }px`,
                        }}
                      >
                        {/* Date Headers */}
                          ...
                          {monthDates.map((date) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className={cellSizes.fontSize}>
                                  {format(date, "d")}
                                </div>
                                <div className="flex flex-col items-center justify-center space-y-1 mt-1">
                                  {/* テンプレート基準の不足警告 */}
                                  {dayStats.hasTemplateData &&
                                  dayStats.isCriticalShortage ? (
                                    <div
                                      className={`flex items-center space-x-1 text-xs text-red-700 bg-red-100 px-1 py-0.5 rounded-full`}
                                    >
                                      <AlertTriangle
                                        className={cellSizes.iconSize}
                                      />
                                      <span>
                                        危険 -{dayStats.templateShortage}
                                      </span>
                                    </div>
                                  ) : dayStats.hasTemplateData &&
                                    dayStats.isWarningShortage ? (
                                    <div
                                      className={`flex items-center space-x-1 text-xs text-yellow-700 bg-yellow-100 px-1 py-0.5 rounded-full`}
                                    >
                                      <AlertTriangle
                                        className={cellSizes.iconSize}
                                      />
                                      <span>
                                        注意 -{dayStats.templateShortage}
                                      </span>
                                    </div>
                                  ) : dayStats.hasTemplateData ? (
                                    <div
                                      className={`flex items-center space-x-1 text-xs text-green-700 bg-green-100 px-1 py-0.5 rounded-full`}
                                    >
                                      <CheckCircle
                                        className={cellSizes.iconSize}
                                      />
                                  ...
                                  {dayStats.hasTemplateData &&
                                    (dayStats.isCriticalShortage ||
                                      ...
                                      <div className="flex space-x-1 text-xs">
                                        {dayStats.templateShortageByTimeSlot
                                          .morning > 0 && (
                                          <span
                                  ...
                                  >
                                    <div className="w-full flex flex-col items-center justify-center space-y-1">
                                      ...
                                      {myShifts.length === 0 && !isPast && (
                                        <button
                                          onClick={() =>
                                            handleCreateShift(date, staffMember)
                                          }
                                          className={`w-full ${
                                            calendarView === "month"
                                              ? "h-6"
                    ...
                    <table className="min-w-full border border-gray-200">
                      <thead>
                        <tr className="bg-gray-50">
                          <th className="border border-gray-200 px-2 py-2 text-center w-20">
                            日付
                          </th>
                      ...
                      <tbody>
                        {monthDates.map((date, index) => {
                          ...
                          return (
                            ...
                            >
                              <td className="border border-gray-200 px-2 py-2 text-center w-20">
            ...
            <div className="relative z-10 bg-white/90 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl max-w-lg w-full max-h-screen overflow-y-auto transform transition-all duration-300 scale-100">
              <div className="p-8">
                ...
                <div className="space-y-6">
                  ...
                  {calendarView === "grid" && !createModalStaff && (
                    <div>
                      ...
                      <div className="border border-gray-200 rounded-xl p-3 max-h-48 overflow-y-auto bg-white/50">
                        {staff.length > 0 ? (
                          <div className="space-y-2">
                            {console.log(
                              "Staff data:",
                              staff.map((s) => ({ name: s.name, uid: s.uid }))
                            )}
                            {console.log(
                            ...
                            {staff.map((staffMember) => (
                              ...
                              >
                                <input
                                  ...
                                  checked={selectedStaffForCalendar.includes(
                                    staffMember.uid
                                  )}
                                  onClick={(e) => {
                                    e.stopPropagation();
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleFormChange("positions", e.target.value)
                          }
                          className="w-full px-4 py-3 bg-white/50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent backdrop-blur-sm transition-all duration-200"
                          placeholder="レジ、フロア、キッチンなど（カンマ区切り）"
                        />
              ...
              {createModalStaff && (
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-xl">
                  <div className="flex items-center space-x-2 text-green-700">
                    ...
                    <p className="text-sm font-medium">
                      このシフトは{createModalStaff.name}
                      さんに直接設定され、すぐにカレンダーに反映されます
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="mt-6 pt-4 border-t border-gray-200">
                ...
                <button
                  ...
                  className="w-full p-3 border-2 border-red-200 rounded-lg hover:border-red-400 hover:bg-red-50 transition-colors text-center group"
                >
